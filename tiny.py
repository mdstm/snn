...
class TinyModel(spaic.Network):
  def __init__(self):
    ...
    self.con0 = spaic.Connection(pre=self.enc, post=self.neg0, link_type='conv', w_mean=0.5,
                                 in_channels=3, kernel_size=(2, 3), padding=1, stride=2)
    self.con1 = spaic.Connection(pre=self.neg0, post=self.neg1, link_type='full', w_mean=0.5,
